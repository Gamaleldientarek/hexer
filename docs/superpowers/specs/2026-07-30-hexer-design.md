...
A Chrome extension that returns the **exact** colour palette of any website by reading its CSS, ranked by how much of the page each colour actually paints.

Free, open source, no account, no server, no network calls. One click, one popup, done.

### Why it exists

Site Palette (90,000 users, 3.8★) charges $2.50/mo–$99 lifetime and requires a MagicLink account for a job that is 100% local computation. Worse, all four of its algorithms run on a **screenshot** — it quantises pixels and never reads the code. So it returns approximated colours from the visible viewport only, with no variable names and no usage data.

Reading the CSS instead gives exact declared values, the whole page rather than the viewport, the custom-property name behind each colour, and a real measure of dominance. That is the entire thesis.
...
| Category | Properties |
|---|---|
| Fills | `background-color`, `background-image` gradient colour stops |
...
| Declared | `--*` custom properties whose value resolves to a colour |
...
1. **Primary:** walk `document.styleSheets`, read rules matching `:root`, `html`, `body`, `[data-theme]`, `[class*=theme]`, and collect declarations starting with `--` whose value parses as a colour. Wrap every `.cssRules` access in `try/catch` — cross-origin sheets throw `SecurityError` and are skipped silently.
...
Counting occurrences is wrong: a 1px border used 640 times is not more important than one full-bleed hero. Every colour record carries a weight in square pixels.
...
box-shadow colour        →  1                              (presence signal only)
```
...
Zero-area elements, `visibility: hidden`, `opacity < 0.05`, `<script>` / `<style>` / `<head>` subtrees, colours with alpha `< 0.05`, and `transparent`. `display: none` is excluded automatically by its zero rect.
...
`count` is not used by the ranking pipeline — weight is. It is carried through purely so the JSON export can report how many elements used each colour.
...
```
parse.js    CSS colour string → { r, g, b, a }. Handles hex 3/4/6/8, rgb(), rgba(),
            hsl(), hsla(), oklch(), color(), and named colours. Drops a < 0.05.

rank.js     Sums weights per colour across all sources, normalises to weightPct
            (% of total painted area), retains the per-source breakdown.

cluster.js  Merges near-identical colours. Distance = Euclidean in OKLab,
...
```
1. Colour matches <meta name="theme-color">          → BRAND   (forced)
...
5. Tie-break when a colour has both text and fill sources: assign to whichever
   source category contributed the larger share of its total weight.
```

Rule 4 encodes the actual definition of an accent: saturated *and* used sparingly. A saturated colour covering 60% of the page is a surface, not an accent.

### `constants.js` — every tunable in one file

```js
CHROMA_BRAND_MIN    = 0.06    // OKLCH chroma floor for BRAND
BRAND_AREA_MAX_PCT  = 20      // above this, a saturated colour is a SURFACE
...
A 6-column CSS grid per group, colours ordered by weight descending. Each colour's column span:

```
share = colour.weight ÷ group.totalWeight
span  = clamp(1, 4, Math.round(share × 6))
```

Spans are proportional to share **of the colour's own group**, never of the whole palette.

**Sizing is per-group, never global.** Globally, white at 47% would swallow the popup and the brand colour at 6.8% would be a speck — backwards, since the brand colour is the payload. Within BRAND, the primary correctly outweighs the secondary.

### Chrome is greyscale — deliberate

Orange `#F83200` appears only on the brand mark, the active tab underline, and focus/selected states. No electric blue anywhere. A colour tool with a colourful UI misreads its own swatches: surrounding hues shift perception. This is a functional constraint, not a stylistic one.
...
| No colours found | "No colours found — the page may still be loading." + Retry button. |
...
| Page with a single colour | Renders one group. No empty-group headings ever render. |
...
| `vars-root.html` | `:root{--brand:#0055FF}` → variable name attached to the colour |
| `nested-bg.html` | Parent background is **not** credited for area its opaque child covers |
| `gradient.html` | 3-stop gradient → all 3 colours found, each ⅓ of the element's area |
| `border-only.html` | Colour used only as a 1px border → classified BORDER, sub-1% |
| `text-heavy.html` | Text colour weight exceeds a small saturated box |
| `shadow-dom.html` | Colours inside an open shadow root are found |
...
| `CHROMA_BRAND_MIN` / `BRAND_AREA_MAX_PCT` mis-tuned → brand colours misfiled | — | Tuned against the fixture suite; both live in `constants.js` as one-line changes. |
...
Adobe `.ase` export · SVG-file export as a first-class output · palette history · cloud sync · accounts · shareable links · Coolors / Google Art Palette integration · a companion Figma plugin for variables · WCAG contrast badges · full-page scroll-and-stitch capture · Firefox port · colour-blindness simulation · palette editing.
...
| Decision | Choice | Why |
|---|---|---|
| Colour source | CSS first, pixels as a second tab | Exact values, whole page, variable names, real dominance |
...
| Dominance sizing scope | Per group | Global sizing buries the brand colour under white |
| Own UI colour | Greyscale, orange accents only | A colourful UI distorts perception of the swatches |
