...
- Create: `tests/fixtures/hero-dominant.html`, `vars-root.html`, `nested-bg.html`, `gradient.html`, `border-only.html`, `text-heavy.html`, `shadow-dom.html`, `cross-origin-css.html`, `perf-5k.html`
...
```js
{
  ...
  stats: { elements, scanned, truncated, stride, sheetsTotal, sheetsBlocked, durationMs },
}
```
...
test('cross-origin-css: a blocked stylesheet does not throw', async ({ page }) => {
  ...
  expect(result.stats.sheetsTotal).toBeGreaterThan(0);
});
...
export function scanPage({ elementCap }) {
  ...
  const VAR_SCOPE =
    /(?:^|,)\s*(?::root|html|body|\[data-theme[^\]]*\]|\.[\w-]*theme[\w-]*)\s*(?:,|$)/i;
  ...
  const walkRules = (rules) => {
    for (const rule of rules) {
      ...
      if (!VAR_SCOPE.test(rule.selectorText)) continue;

      for (const prop of rule.style) {
        if (!prop.startsWith('--') || seenVars.has(prop)) continue;
        const resolved = resolveColour(rule.style.getPropertyValue(prop));
        if (resolved) { vars.push({ name: prop, value: resolved }); seenVars.add(prop); }
      }
    }
  };
  ...
  for (const sheet of document.styleSheets) {
    sheetsTotal++;
    let rules;
    try { rules = sheet.cssRules; } catch (e) { sheetsBlocked++; continue; }
    if (rules) walkRules(rules);
  }

  // Supplement: Chrome's Typed OM does enumerate custom properties, unlike
  // getComputedStyle iteration.
  try {
    for (const [prop, value] of document.documentElement.computedStyleMap()) {
      if (!prop.startsWith('--') || seenVars.has(prop)) continue;
      const resolved = resolveColour(String(value));
      if (resolved) { vars.push({ name: prop, value: resolved }); seenVars.add(prop); }
    }
  } catch (e) { /* Typed OM unavailable — stylesheet walk already ran */ }
