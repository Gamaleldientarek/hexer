...
<style>
  :root{--brand:#0055FF;--ink:#111111;--not-a-colour:12px}
  html,body{margin:0}
...
`vars-noise.html` — mirrors what Spike A found on real sites: a handful of colour
variables buried in hundreds of non-colour ones.
```html
...
test('vars-root: non-colour custom properties are rejected without probing', async ({ page }) => {
...
export function scanPage({ elementCap }) {
  ...
  for (const [prop, value] of document.documentElement.computedStyleMap()) {
    ...
    // colours: --Layout-column-gap, --animate-bounce, --100dvh. github.com has
    // 1,992. Each probe forces a style recalculation, so reject anything that
    // cannot be a colour before probing.
...
git commit -m "feat: harvest page colours weighted by painted area"
```
...
5. Expected: the header reads `stripe.com` with a colour count, and the body shows grouped JSON with `hex`, `weightPct`, `role`, and some non-null `varName` values.
...
3. Check each of these:
   ...
   - Within brand, the primary colour's swatch is visibly wider than the secondary's.
