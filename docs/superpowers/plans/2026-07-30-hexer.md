...
**Goal:** Build a free MV3 Chrome extension that returns a website's exact colour palette by reading its CSS, ranked by painted area and grouped into brand/text/surface/border.

**Architecture:** A popup page does all UI and all computation. It injects one self-contained function into the active tab to harvest colour records, then runs a pure pipeline (parse → rank → cluster → roles) over the result. A second tab lazily screenshots the viewport and median-cuts the pixels. No service worker, no build step, no network calls.
...
- Colour identity is always the **exact declared value**, never an average.
...
| `src/scan/css-scan.js` | Injected harvester. Walks DOM, returns aggregated colour records. |
| `src/scan/pixel-scan.js` | Screenshot → OffscreenCanvas → quantise. |
| `src/core/constants.js` | Every tunable threshold. |
| `src/core/parse.js` | Colour string → `{r,g,b,a}`; `toHex`. |
| `src/core/oklab.js` | sRGB↔OKLab↔OKLCH, `deltaEok`. |
| `src/core/rank.js` | Aggregate weights per colour, normalise to percentages. |
| `src/core/cluster.js` | Merge near-identical colours, keep heaviest as representative. |
| `src/core/roles.js` | Classify one colour into brand/text/surface/border. |
...
```json
{
  ...
  "private": true,
  "type": "module",
  "description": "Get the exact colour palette of any website, straight from its CSS.",
  "license": "MIT",
  "scripts": {
    "test": "vitest run",
    "test:watch": "vitest",
...
Perceptual colour space. Every later task depends on it, so it goes first.
...
git commit -m "feat: add colour parsing and central constants"
```
...
git commit -m "feat: rank colours by painted-area weight"
```
...
git commit -m "feat: merge near-identical colours in OKLab"
```
...
git commit -m "feat: classify colours into brand/text/surface/border"
```
