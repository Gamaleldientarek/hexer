...
On tailwindcss.com, **222 of 225 distinct colours failed to parse** — Tailwind v4
emits `oklab()` and `lab()`, and Chrome serialises modern colour functions in
their own space rather than converting to `rgb()`. The scan returned 3 colours
...
**Effect on tailwindcss.com: 3 colours → 204.**

## Success criterion 1: NOT MET

> On 10 well-known sites, the site's true primary brand colour lands in the
...
| figma.com | `#F24E1E` | `#CB9FD2` `#00B6FF` `#24CB71` | fail — only 8 colours from 1,225 elements |
| spotify.com | `#1DB954` | **`#1ED760`** `#AF2896` `#509BF5` | **pass** — current brand green, top 1 |
| airbnb.com | `#FF5A5F` | **`#FF385C`** `#DA1249` `#0066FF` | **pass** — current brand colour, top 1 |
| gamaleldien.com | `#F83200` | `#E16105` `#40CE03` | fail |

### Why, and why tuning will not close it

The brand colours *are* found — they are in the palette. They rank low because
painted area is the wrong signal for *primacy*: a primary brand colour often
...
primary brand colour recoverable from CSS. Three of the ten do not have a single
chromatic brand colour at all (Vercel, Notion, Figma), and no amount of
...
| Completeness | 204 colours on tailwind, 98 on stripe, 64 on linear |
| Speed | 17–113 ms across all ten sites, all well inside the 400 ms budget |
| Neutrals separated from saturated colours | white/near-black consistently in surface, never brand |
...
returned 11 elements and 4 colours before hydration. `www.spotify.com` is
stable.
